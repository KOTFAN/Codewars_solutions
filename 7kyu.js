...
function findMiddle(str) {
  ...
  if (intsArr.length > 0) {
    ...
    const res =
      prod.length > 2
        ? parseInt(prod.slice(delCount).slice(0, midleLen))
        : parseInt(prod);
...
function getMiddle(s) {
  ...
  return sL % 2 === 0
    ? s.slice(Math.floor(sL / 2 - 1), -Math.floor(sL / 2 - 1))
    : s.slice(Math.floor(sL / 2), -Math.floor(sL / 2));
}
...
function XO(str) {
  return !str
    ...
    .reduce(
      (counter, value) =>
        value === "x" ? counter + 1 : value === "o" ? counter - 1 : counter,
      0
    );
}
...
function printerError(s) {
  return `${s
    .split("")
    .reduce((c, v) => (v.charCodeAt() < 110 ? c + 0 : c + 1), 0)}/${s.length}`;
}
...
function newAvg(arr, newavg) {
  const donation = Math.ceil(
    newavg * (arr.length + 1) - arr.reduce((a, v) => a + v, 0)
  );
...
function validatePIN(pin) {
  ...
  if (
    ...
    pin.split("").every((n) => nums.includes(n))
  ) {
    return true;
  }
  return false;
...
function applyFeedback(battlescape, enemy, position) {
  const emojiArr = splitEmoji(battlescape);
  if (emojiArr.indexOf(enemy) === position) return DISPENSE_TREAT;
