...
function duplicateCount(text) {
  ...
  text
    .split("")
    .forEach((l) => (onlyOnes.has(l) ? dublicates.add(l) : onlyOnes.add(l)));
...
function alphabetPosition(text) {
  return text
    ...
    .split("")
    .map((s) =>
      s.charCodeAt() > 64 && s.charCodeAt() < 91 ? s.charCodeAt() - 64 : 0
    )
    .filter((v) => v > 0)
    .join(" ");
}
...
function narcissistic(value) {
  ...
  return (
    ...
    value
      .toString()
      .split("")
      .reduce((a, v) => a + Number(v) ** numLen, 0)
  );
}
...
function isPangram(string) {
  ...
  alphabet.forEach((l) =>
    strChars.has(l) ? l : (isHasAllAlphabetLetters = false)
  );
...
function sortArray(array) {
  ...
  for (
    let i = 0, oddIndex = oddIndexes[0];
    i < oddNums.length;
    i++, oddIndex = oddIndexes[i]
  ) {
...
function deleteNth(arr, n) {
  //every element should be only no more than n-times
...
function countSmileys(arr) {
  ...
  const count = arr.reduce((numOfValid, face) => {
    ...
    if (splitedFace.length === 2) {
      ...
      //Eyes and Smiling
      if (
        validEyes.includes(splitedFace[0]) &&
        validSmiling.includes(splitedFace[1])
      ) {
        return numOfValid + 1;
      }
    }
